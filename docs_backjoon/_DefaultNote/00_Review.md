...
```cpp
- int: 21억의 제곱근은 약 4만 6천 (46,340.xx)
- unsigned int: 42억의 제곱근은 약 6만 5천 (65,535.xx)
- long long : 922경의 제곱근은 약 30억 (3,037,000,499.xx)
- unsigned long long: 1,844경의 제곱근은 약 42억 (4,294,967,295.xx)
```

- 위와 같이 `4byte`자료형의 제곱근의 범위는 생각보다 작으므로 곱셈 대상이 되는 원소의 크기를 보고 위 값 이상일 경우 반드시 연산을 하는` R-Value`부터 승격시켜야함
	- 정수형은 `long long` 사용
	- 실수형은 `double` 사용
	-  `int`로 `long long`을 곱셈할 경우 `1LL * a * b` 와 같은 방식으로 급셈
	- `float`로 `double`을 곱셈할 경우 `1.0 * a * b` 와 같은 방식으로 곰셈
  
```cpp
int a = 100000, b = 100000;
long long x = a * b;          // [X] a*b가 int로 먼저 계산되어 오버플로우 가능
long long y = 1LL * a * b;    // [O] 처음부터 long long로 승격
```
